...
# الإصدار 2.0 - مع إضافة يدوي وإرسال تلقائي كل يومين
...
# =========================================================
# 🤖 إعدادات البوت المستقبل للنسخ الاحتياطية التلقائية
# =========================================================

BACKUP_BOT_TOKEN = "8774063852:AAFBNCRbFlJMwJWDsQudUBtfZy3iC8Smbhk"
BACKUP_CHAT_ID = None  # سيتم تحديده تلقائياً عند الإرسال
...
def get_vault_stats() -> Dict:
    ...
    # إحصائيات التوكن
    cursor.execute("SELECT COUNT(*) FROM vault_accounts WHERE token_status = 'VALID'")
    valid_count = cursor.fetchone()[0]
    
    cursor.execute("SELECT COUNT(*) FROM vault_accounts WHERE token_status = 'EXPIRED'")
...
async def add_manual_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # بدء حالة الإضافة اليدوية
    user_steps[user_id] = {
        "step": "add_manual_name",
        "data": {}
    }
    
    await query.message.edit_text(
        "➕ **إضافة حساب يدوي**\n\n"
        "📌 **الخطوة 1/8:** أرسل **اسم الحساب**.\n\n"
        "💡 مثال: `حساب رئيسي`\n\n"
...
async def cancel_manual_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
...
async def handle_manual_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # التحقق من وجود عملية إضافة يدوي
    ...
    # إذا كانت الخطوة لا تبدأ بـ "add_manual" نخرج
    if not step or not step.startswith("add_manual"):
        return
    ...
    # تخزين البيانات حسب الخطوة
    if step == "add_manual_name":
        step_data["data"]["name"] = text
        step_data["step"] = "add_manual_id"
        await update.message.reply_text(
            "➕ **إضافة حساب يدوي**\n\n"
            "📌 **الخطوة 2/8:** أرسل **ID الحساب**.\n\n"
            "💡 مثال: `123456789`\n\n"
            "🔙 لإلغاء العملية أرسل `/cancel`",
            parse_mode="Markdown"
        )
    
    elif step == "add_manual_id":
        step_data["data"]["account_id"] = text
        step_data["step"] = "add_manual_server"
        await update.message.reply_text(
            "➕ **إضافة حساب يدوي**\n\n"
            "📌 **الخطوة 3/8:** أرسل **السيرفر**.\n\n"
            "💡 مثال: `ME` أو `EU` أو `US`\n"
            "📌 اتركها فارغة للافتراضي `ME`\n\n"
            "🔙 لإلغاء العملية أرسل `/cancel`",
            parse_mode="Markdown"
        )
    
    elif step == "add_manual_server":
        if text.strip():
            step_data["data"]["server"] = text.strip()
        else:
            step_data["data"]["server"] = "ME"
        step_data["step"] = "add_manual_prime"
        await update.message.reply_text(
            "➕ **إضافة حساب يدوي**\n\n"
            "📌 **الخطوة 4/8:** أرسل **مستوى Prime** (1-8).\n\n"
            "💡 مثال: `4`\n\n"
            "🔙 لإلغاء العملية أرسل `/cancel`",
            parse_mode="Markdown"
        )
    
    elif step == "add_manual_prime":
        try:
            prime = int(text.strip())
            if prime < 1 or prime > 8:
                raise ValueError("Prime خارج النطاق")
            step_data["data"]["prime_level"] = prime
            step_data["step"] = "add_manual_security_code"
            await update.message.reply_text(
                "➕ **إضافة حساب يدوي**\n\n"
                "📌 **الخطوة 5/8:** أرسل **رمز الأمان** (6 أرقام).\n\n"
                "💡 مثال: `123456`\n"
                "📌 اكتب `لا يوجد` إذا لم يكن هناك رمز\n\n"
                "🔙 لإلغاء العملية أرسل `/cancel`",
                parse_mode="Markdown"
            )
        except ValueError:
            await update.message.reply_text(
                "❌ **خطأ:** الرجاء إدخال رقم بين 1 و 8.\n\n"
                "📌 **الخطوة 4/8:** أرسل **مستوى Prime** (1-8).",
                parse_mode="Markdown"
            )
    
    elif step == "add_manual_security_code":
        code = text.strip()
        if code.lower() == "لا يوجد":
            step_data["data"]["security_code"] = None
        else:
            # التحقق من أن الرمز 6 أرقام
            if not re.match(r'^\d{6}$', code):
                await update.message.reply_text(
                    "❌ **خطأ:** رمز الأمان يجب أن يكون 6 أرقام.\n\n"
                    "📌 **الخطوة 5/8:** أرسل **رمز الأمان** (6 أرقام).\n"
                    "💡 مثال: `123456`\n"
                    "📌 اكتب `لا يوجد` إذا لم يكن هناك رمز",
                    parse_mode="Markdown"
                ...
                return
            step_data["data"]["security_code"] = code
        step_data["step"] = "add_manual_bound_email"
        await update.message.reply_text(
            "➕ **إضافة حساب يدوي**\n\n"
            "📌 **الخطوة 6/8:** أرسل **البريد المرتبط**.\n\n"
            "💡 مثال: `user@gmail.com`\n"
            "📌 اتركها فارغة إذا لا يوجد\n\n"
            "🔙 لإلغاء العملية أرسل `/cancel`",
            parse_mode="Markdown"
        )
    
    elif step == "add_manual_bound_email":
        if text.strip():
            step_data["data"]["bound_email"] = text.strip()
        else:
            step_data["data"]["bound_email"] = "لا توجد"
        step_data["step"] = "add_manual_pending_email"
        await update.message.reply_text(
            "➕ **إضافة حساب يدوي**\n\n"
            "📌 **الخطوة 7/8:** أرسل **البريد الجديد** (Pending Email).\n\n"
            "💡 مثال: `new@gmail.com`\n"
            "📌 اتركها فارغة إذا لا يوجد\n\n"
            "🔙 لإلغاء العملية أرسل `/cancel`",
            parse_mode="Markdown"
    ...
    elif step == "add_manual_pending_email":
        if text.strip():
            step_data["data"]["pending_email"] = text.strip()
        else:
            step_data["data"]["pending_email"] = "لا توجد"
        step_data["step"] = "add_manual_token_status"
        await update.message.reply_text(
            "➕ **إضافة حساب يدوي**\n\n"
            "📌 **الخطوة 8/8:** أرسل **حالة التوكن**.\n\n"
            "💡 اختر أحد الخيارين:\n"
            "• `VALID` - صالح ✅\n"
            "• `EXPIRED` - محروق 🔴\n\n"
            "🔙 لإلغاء العملية أرسل `/cancel`",
    ...
    elif step == "add_manual_token_status":
        status = text.strip().upper()
        if status not in ["VALID", "EXPIRED"]:
            await update.message.reply_text(
                "❌ **خطأ:** الرجاء إدخال `VALID` أو `EXPIRED` فقط.\n\n"
                "📌 **الخطوة 8/8:** أرسل **حالة التوكن**.\n"
                "💡 اختر أحد الخيارين:\n"
                "• `VALID` - صالح ✅\n"
                "• `EXPIRED` - محروق 🔴",
        ...
        step_data["data"]["token_status"] = status
        step_data["data"]["eat_token"] = "غير متوفر"
        step_data["data"]["recovery_end_time"] = None
        step_data["data"]["extra_data"] = {}
        
        # إضافة الحساب إلى قاعدة البيانات
        account_data = step_data["data"]
        result = add_vault_account(account_data)
        ...
        if result > 0:
            await update.message.reply_text(
                ...
                f"🆔 **ID:** `{account_data.get('account_id')}`\n"
                f"🌍 **السيرفر:** {account_data.get('server')}\n"
                f"🏆 **Prime:** {account_data.get('prime_level')}\n"
                f"🔐 **رمز الأمان:** `{account_data.get('security_code', 'لا يوجد')}`\n"
                f"📧 **البريد:** {account_data.get('bound_email')}\n"
                f"🔑 **حالة التوكن:** {status}\n\n"
                f"📌 يمكنك عرضه من قائمة الحسابات.",
                reply_markup=main_menu_keyboard(),
                parse_mode="Markdown"
            )
        else:
            await update.message.reply_text(
                f"❌ **فشل إضافة الحساب!**\n\n"
                f"⚠️ قد يكون ID الحساب `{account_data.get('account_id')}` مكرراً.\n\n"
                f"📌 حاول مرة أخرى أو استخدم زر الإضافة من القائمة.",
                reply_markup=main_menu_keyboard(),
                parse_mode="Markdown"
            )
...
async def show_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text += f"\n🔑 **حالة التوكن:**\n"
    text += f"└── 🟢 صالح: {stats['valid_count']}\n"
    text += f"└── 🔴 محروق: {stats['expired_count']}"
...
async def auto_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_steps:
        ...
        if step == "waiting_backup":
            ...
            return
        elif step.startswith("add_manual"):
...
# =========================================================
# 🤖 إرسال النسخة الاحتياطية التلقائية كل يومين
# =========================================================

async def send_auto_backup(context: ContextTypes.DEFAULT_TYPE):
    """إرسال نسخة احتياطية تلقائية إلى البوت الثاني كل يومين"""
    logger.info("📤 بدء إرسال النسخة الاحتياطية التلقائية...")
    ...
    try:
        ...
        if not accounts:
            logger.info("📭 لا توجد حسابات لإرسالها")
            return
        ...
        export_data = {
            "backup_meta": {
                ...
                "created_at": datetime.now().isoformat(),
                "total_accounts": len(accounts),
                "source": "Vault Bot (نسخة تلقائية كل يومين)"
            },
            "accounts": accounts,
            "vault_export": True
        }
        ...
        for p in range(1, 9):
            emoji = get_prime_emoji(p)
            count = stats['prime_stats'].get(p, 0)
            prime_text += f"└── {emoji} Prime {p}: {count}\n"
        
        caption = (
            f"📦 **نسخة احتياطية تلقائية**\n\n"
            f"📅 **التاريخ:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
            f"📦 **عدد الحسابات:** {stats['total']}\n\n"
            f"🏆 **توزيع Prime:**\n{prime_text}\n"
            f"🔑 **حالة التوكن:**\n"
            f"└── 🟢 صالح: {stats['valid_count']}\n"
            f"└── 🔴 محروق: {stats['expired_count']}\n\n"
            f"📌 *هذه نسخة احتياطية تلقائية ترسل كل يومين.*"
        )
        
        # إرسال الملف إلى البوت الثاني
        with open(zip_filename, 'rb') as f:
            await context.bot.send_document(
                chat_id=BACKUP_CHAT_ID if BACKUP_CHAT_ID else "@YourBackupBot",
                document=f,
                filename=zip_filename,
                caption=caption,
                parse_mode="Markdown"
        ...
        logger.info(f"✅ تم إرسال النسخة الاحتياطية التلقائية بنجاح! عدد الحسابات: {stats['total']}")
        
    except Exception as e:
        logger.error(f"❌ خطأ في إرسال النسخة الاحتياطية التلقائية: {e}")
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        await add_manual_start(update, context)
...
async def post_init(application: Application):
    ...
    # ✅ جدولة إرسال النسخة الاحتياطية التلقائية كل يومين الساعة 10 مساءً
    # توقيت مصر/السعودية (UTC+3)
    tz = pytz.timezone('Asia/Riyadh')
    now = datetime.now(tz)
    
    # تحديد موعد الساعة 10 مساءً اليوم
    target_time = now.replace(hour=22, minute=0, second=0, microsecond=0)
    
    # إذا كان الوقت الحالي بعد 10 مساءً، نحدد الموعد لليوم التالي
    if now > target_time:
    ...
    # جدولة المهمة: تعمل كل 172800 ثانية (يومين) بعد أول تنفيذ
    job_queue = application.job_queue
    if job_queue:
        # تشغيل لأول مرة عند الوصول للوقت المحدد
        job_queue.run_once(send_auto_backup, seconds_until_target)
        # ثم التكرار كل يومين
        job_queue.run_repeating(send_auto_backup, interval=172800, first=seconds_until_target + 172800)
        logger.info(f"⏰ تم جدولة النسخ الاحتياطي التلقائي كل يومين الساعة 10 مساءً (بتوقيت السعودية)")
        logger.info(f"📅 أول نسخة في: {target_time.strftime('%Y-%m-%d %H:%M:%S')}")
    else:
        logger.warning("⚠️ JobQueue غير متاح، لن تعمل النسخ التلقائي")
...
def main():
    ...
    app.add_handler(CommandHandler("cancel", cancel_manual_add))
    ...
    logger.info("➕ زر الإضافة اليدوي مفعل")
    logger.info("⏰ الإرسال التلقائي كل يومين الساعة 10 مساءً (بتوقيت السعودية)")
